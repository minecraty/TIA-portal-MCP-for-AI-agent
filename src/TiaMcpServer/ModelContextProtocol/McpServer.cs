...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    public static class McpServer
    {
        ...
        [McpServerTool(Name = "CreateDevice"), Description("Create a new device in the project")]
        public static ResponseDeviceInfo CreateDevice(
            [Description("typeId: the device type identifier (e.g., 'OrderNumber:6ES7 214-1AG40-0XB0' for S7-1200 CPU 1214C)")] string typeId,
            [Description("deviceName: the name for the new device")] string deviceName,
            [Description("itemTypeName: optional item type name for CreateWithItem")] string? itemTypeName = null)
        {
            try
            {
                var device = Portal.CreateDevice(typeId, deviceName, itemTypeName);
                if (device != null)
                {
                    var attributes = Helper.GetAttributeList(device);
                ...
                {
                    throw new McpException($"Failed to create device '{deviceName}'", McpErrorCode.InternalError);
                }
            }
            catch (Exception ex) when (ex is not McpException)
            {
            ...
            [Description("password: the password to access adminsitration, default: no password")] string password = "")
        {
            try
            {
                ...
                if (result != null)
                {
                    ...
                    errorMessages.AppendLine($"State: {result.State}");
                    ...
                    {
                        errorMessages.AppendLine("  No detailed messages available.");
                    }

                    if (!result.State.ToString().Equals("Error"))
                    {
                        return new ResponseCompileSoftware
                        {
                            Message = $"Software '{softwarePath}' compiled.\n{errorMessages}",
                            Meta = new JsonObject
                            {
                                ["timestamp"] = DateTime.Now,
                                ["success"] = true,
                    ...
                    {
                        throw new McpException($"Failed compiling software '{softwarePath}':\n{errorMessages}", McpErrorCode.InternalError);
                    }
                }
                else
                {
                    throw new McpException($"Failed compiling software '{softwarePath}': No result returned", McpErrorCode.InternalError);
                }
            }
            catch (Exception ex) when (ex is not McpException)
            {
        ...
        [McpServerTool(Name = "ImportExternalSource"), Description("Import SCL source file (.scl) into PLC software external source group (V20+)")]
        public static ResponseImportExternalSource ImportExternalSource(
            [Description("softwarePath: defines the path in the project structure to the plc software")] string softwarePath,
            [Description("importPath: defines the path of the .scl file to import")] string importPath)
        {
            try
            {
                if (Engineering.TiaMajorVersion < 20)
                {
                    throw new McpException("ImportExternalSource requires TIA Portal V20 or newer", McpErrorCode.InvalidParams);
                }

                if (Portal.ImportExternalSourceAndGenerate(softwarePath, importPath))
                {
        ...
        [McpServerTool(Name = "GenerateBlocksFromSource"), Description("Generate program blocks from an external source file already imported into PLC software (V20+)")]
        public static ResponseGenerateBlocksFromSource GenerateBlocksFromSource(
            [Description("softwarePath: defines the path in the project structure to the plc software")] string softwarePath,
            [Description("sourceName: name of the external source file (without extension) to generate blocks from")] string sourceName)
        {
            try
            {
                if (Engineering.TiaMajorVersion < 20)
                {
                    throw new McpException("GenerateBlocksFromSource requires TIA Portal V20 or newer", McpErrorCode.InvalidParams);
                }

                if (Portal.GenerateBlocksFromExternalSource(softwarePath, sourceName))
                {
            ...
            {
                throw new McpException($"Unexpected error generating blocks from '{sourceName}': {ex.Message}", ex, McpErrorCode.InternalError);
            }
        }
        ...
        [McpServerTool(Name = "ExportTagTable"), Description("Export a PLC tag table from the plc software to path")]
        public static ResponseExportTagTable ExportTagTable(
            [Description("softwarePath: defines the path in the project structure to the plc software")] string softwarePath,
            [Description("exportPath: defines the path where to export the tag table")] string exportPath,
            [Description("tagTableName: name of the tag table to export (empty for default)")] string tagTableName = "",
            [Description("preservePath: preserves the path/structure of the plc software")] bool preservePath = false)
        {
            try
            {
                var success = Portal.ExportTagTable(softwarePath, tagTableName, exportPath, preservePath);
                if (success)
                {
                    return new ResponseExportTagTable
                    {
                        Message = $"Tag table exported to '{exportPath}'",
                        Meta = new JsonObject
                        {
                            ["timestamp"] = DateTime.Now,
                            ["success"] = true
                ...
                throw new McpException($"Failed exporting tag table from '{softwarePath}' to '{exportPath}'",
                    McpErrorCode.InternalError);
            }
            catch (Exception ex) when (ex is not McpException)
            {
                throw new McpException($"Unexpected error exporting tag table: {ex.Message}", ex, McpErrorCode.InternalError);
            }
        }

        [McpServerTool(Name = "ImportTagTable"), Description("Import a PLC tag table from file into plc software")]
        public static ResponseImportTagTable ImportTagTable(
            [Description("softwarePath: defines the path in the project structure to the plc software")] string softwarePath,
            [Description("importPath: defines the path of the XML file to import the tag table from")] string importPath)
        {
            try
            {
                var success = Portal.ImportTagTable(softwarePath, importPath);
                if (success)
                {
                ...
                throw new McpException($"Failed importing tag table from '{importPath}' to '{softwarePath}'",
                    McpErrorCode.InternalError);
            }
            catch (Exception ex) when (ex is not McpException)
            {
                throw new McpException($"Unexpected error importing tag table: {ex.Message}", ex, McpErrorCode.InternalError);
            }
        }
